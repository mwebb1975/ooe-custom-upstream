...
class OW_Tools_Service {
   ...
   public function owf_imports() {
      ...
      foreach( $json as $key=>$values ) {  
         ...
         if ( $key == "teams" && is_plugin_active( 'ow-teams/ow-teams.php' ) ) {
            ...
            $new_team_data = $this->import_teams( $values );
            OW_Utility::instance()->logger( "OWF imported teams" );
         }
         ...
         if ( $key == "groups" && is_plugin_active( 'ow-groups/ow-groups.php' ) ) {
            ...
            $new_group_data = $this->import_groups( $values );
            OW_Utility::instance()->logger( "OWF imported groups" );
         }
         ...
         if( $key == "workflows" ) {
            ...
            if ( isset( $new_team_data ) && ! empty( $new_team_data ) ) {
               $replace_team_ids = $new_team_data;
            } 
            
            if ( isset( $new_group_data ) && ! empty( $new_group_data ) ){
               $replace_group_ids = $new_group_data;
            } 
            
            // Get new inserted workflow ids to replace it with old ids during workflow settings import
            $new_workflow_data = $this->import_workflows( $values, $replace_team_ids, $replace_group_ids );
            
            OW_Utility::instance()->logger( "OWF imported workflows" );
         }
         
         // import settings
         if ( $key == "settings" ) {
            $replace_workflow_ids = array();
            if ( isset( $new_workflow_data ) && ! empty( $new_workflow_data ) ){
               $replace_workflow_ids = $new_workflow_data;
            } 
            $this->import_settings( $values, $replace_workflow_ids );
            OW_Utility::instance()->logger( "OWF imported settings" );
         }         
      }
      
      if ( $json ) {
         add_action ( 'admin_notices', array ( $this, 'data_imported_successfully_notice' ) );
	    	return FALSE;
      }
   }
    ...
    * @since 5.3
    */
   public function import_workflows( $workflow_data, $replace_team_ids, $replace_group_ids ) {
      global $wpdb;
      // get the table names
    ...
    * @since 5.3
    */
   public function import_teams( $team_data ) {
      global $wpdb;
      // get the table names
    ...
    * @since 5.3
    */
   public function import_groups( $group_data ) {
      global $wpdb;
      // get the table names
    ...
    * @since 5.3
    */
   public function import_settings( $settings_data, $replace_workflow_ids ) {
