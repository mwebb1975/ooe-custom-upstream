...
Events Manager
					...
					<td colspan="2">
						...
						if ( !is_ssl() ){
							...
							echo sprintf( esc_html__('Your site is not using SSL! Whilst not a requirement, if you\'re going to submit API information for a live PayPal account, we recommend you do so over a secure connection. If this is not possible, consider an alternative option of submitting your API information as covered in our %s.', 'em-pro'),
										'<a href="http://wp-events-plugin.com/documentation/events-with-paypal/">'.esc_html__('documentation','events-manager').'</a>');
							echo '</p>';
						}
						?>
					</td>
				</tr>
		  		<?php em_options_input_text( esc_html__('API Username', 'em-pro'), 'em_'. $this->gateway . '_api_username' ); ?>
		  		<?php em_options_input_text( esc_html__('API Password', 'em-pro'), 'em_'. $this->gateway . '_api_password' ); ?>
		  		<?php em_options_input_text( esc_html__('API Signature', 'em-pro'), 'em_'. $this->gateway . '_api_signature' ); ?>
			</tbody>
		</table>
	...
	function update() {
		...
		$gateway_options[] = 'em_'. $this->gateway . '_api_username';
		$gateway_options[] = 'em_'. $this->gateway . '_api_password';
		$gateway_options[] = 'em_'. $this->gateway . '_api_signature';
...
function em_gateway_paypal_booking_timeout(){
	global $wpdb;
	//get creds and check they exist before even trying this
	$api_user = get_option('em_paypal_api_username');
	$api_pass = get_option('em_paypal_api_password');
	$api_sig = get_option('em_paypal_api_signature');
