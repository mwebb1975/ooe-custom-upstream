...
class EM_Gateways {
	...
	static function init(){
		...
		// Payment return - backwards compatible with EMP < 2.4.4.4
		...
		add_action('parse_query', 'EM_Gateways::handle_payment_gateways');
		add_filter('rewrite_rules_array','EM_Gateways::handle_payment_rewrite_rule');
		add_filter('query_vars', 'EM_Gateways::handle_payment_query_var');
		//Booking Tables UI
	 ...
	 * This function works for both old and new address permalinks like so:
	 * 
	 *  yoursite.com/wp-admin/admin-ajax.php?action=em_payment&em_payment_gateway=gatewayname
	 *  yoursite.com/em-payment-gateway-gatewayname/
	 */
	public static function handle_payment_gateways() {
	    //Thank you PayPal for forcing this hacky workaround upon us....
	    if( !empty($_GET['action']) && $_GET['action'] == 'em_payment' && empty($_GET['em_payment_gateway']) ) $_REQUEST['em_payment_gateway'] = 'paypal';
	    //Listen on admin-ajax.php or non-permalink setups as well as new permalink style for callbacks
		if( !empty($_REQUEST['em_payment_gateway']) ) {
			do_action( 'em_handle_payment_return_' . $_REQUEST['em_payment_gateway']);
			exit();
		}elseif( get_query_var('em_payment_gateway', false) ){
		    do_action( 'em_handle_payment_return_' . get_query_var('em_payment_gateway') );
		    exit();
		}
	}

	/**
	 * Adds rewrite rule for gateway notification listeners in the rewrite_rules_array filter
	 * @param array $rules
	 * @return array
	 */
	public static function handle_payment_rewrite_rule( $rules ) {
	    $new_rule = array();
	    $new_rule['em-payment-gateway-(.+)/?$'] = 'index.php?em_payment_gateway=$matches[1]';
	    return $new_rule + $rules;
	}
	
	/**
	 * Adds the em_payment_gateway query variable during the query_vars filter.
	 * @param array $vars
	 * @return array
	 */
	public static function handle_payment_query_var( $vars ){
